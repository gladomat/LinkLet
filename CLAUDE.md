...
When a task may involve repeatable tool use, prior debugging paths, API exploration, repo investigation, or multi-step ops, consider loading the `recall-workflow` skill.

Use it to avoid rediscovering known paths: recall prior traces, reuse crystallized workflows/specs when available, and record useful successful or failed runs for future reuse. Skip for simple one-off tasks.
...
The project is indexed. The canonical top-level Child DOX Index lives in the root `AGENTS.md` (the DOX rail) — read it there; do not duplicate it here. Child docs: `data/`, `data/index/`, `data/sync/`, `ui/` (under `app/src/main/java/com/gladomat/linklet/`), plus `tests/` and `docs/`.
